...
# ablation 조건들. use_offer, use_negotiation, use_human_query 플래그만 바꿔서 실행
ABLATION_CONDITIONS = [
...
def _save_result(result, condition, pt, tc, run_idx):
...
def run_ablation_study(task_id, image_pairs):
    ...
    all_rows = {cond: [] for cond, _ in ABLATION_CONDITIONS}
    ...
    for run_idx, (img_a, img_b) in enumerate(image_pairs, 1):
        ...
        for condition, flags in ABLATION_CONDITIONS:
            ...
            tracker.start()
            try:
                result = run(task_id=task_id, img_a=img_a, img_b=img_b, label=condition, **flags)
    ...
    for condition, _ in ABLATION_CONDITIONS:
        ...
        final_rows.append({"Condition": condition, "PT(s)": round(float(pt_avg), 2), "TC": int(tc_avg)})
